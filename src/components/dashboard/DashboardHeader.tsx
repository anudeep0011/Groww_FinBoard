...
import { Download, LayoutTemplate, User, LogOut, Settings } from "lucide-react";
...
export function DashboardHeader() {
    const { widgets, theme, dataSource, setDataSource, importDashboard, switchTemplate, activeTemplate } = useDashboardStore();
    const { user, logout } = useAuth();

    // Unified state: null = closed, "profile" | "settings" = open with tab
    const [configTab, setConfigTab] = React.useState<"profile" | "settings" | null>(null);
    ...
    return (
        <header className="px-6 py-4 flex items-center justify-between bg-card/80 backdrop-blur-md border-b border-border/40 sticky top-0 z-40 shadow-sm">
            ...
            <div className="relative group">
                ...
                <div className="absolute top-full right-0 pt-2 w-56 hidden group-hover:block animate-in fade-in zoom-in-95 duration-100 z-50">
                    <div className="bg-card border border-border rounded-lg shadow-xl overflow-hidden p-1">
                        ...
                        <button
                            onClick={() => {
                                setDataSource("FINNHUB"); // Force Real Data
                                switchTemplate('TECH', [
            ...
            <div className="relative group ml-2">
                <button className="flex items-center justify-center w-8 h-8 rounded-full bg-muted/50 hover:bg-muted border border-border/50 transition-colors overflow-hidden">
                    {user?.photoURL ? (
                ...
                <div className="absolute top-full right-0 pt-2 w-48 hidden group-hover:block animate-in fade-in zoom-in-95 duration-100 z-50">
                    <div className="bg-card border border-border rounded-lg shadow-xl overflow-hidden">
                        ...
                        <div className="p-1">
                            <button
                                ...
                                className="w-full text-left px-2 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded-md flex items-center gap-2 transition-colors"
                            >
                        ...
                        <div className="p-1">
                            <button
                                onClick={() => logout()}
                                className="w-full text-left px-2 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/10 rounded-md flex items-center gap-2 transition-colors"
                            >
